# T-05: RLS Policies — Implementation Plan

All decisions locked via grill Q10, Q14–Q19, Q20b.

---

## Migration File

`supabase/migrations/20260511000001_rls_policies.sql`
...
## Section 2: Helper Functions (5 total)
...
## Section 4: Policies by Table

### profiles (Pattern: read-all, write-own + admin override)
...
### work_experiences, educations, certificates (Pattern: read-all, write-own)
...
### job_categories, skills (Pattern: read-only lookup)
...
### user_skills (Pattern: read-all, write-own)
...
### job_required_skills (Pattern: recruiter-owned via job chain)
...
### companies (Pattern: read-all, write-own + admin)
...
### job_posts (Pattern: role-based visibility — Q15)
...
### job_locations (Pattern: follows job_posts)
...
### applications (Pattern: cross-role — Q17)
...
### application_notes (Pattern: recruiter-only, admin read)
...
### interview_schedules (Pattern: recruiter creates, seeker reads)

> **Note:** SELECT policies use uncached subqueries on applications.
> Acceptable at portfolio scale. Flag for T-38 optimization if needed.
...
### conversations (Pattern: participant-based — Q18)
...
### messages (Pattern: participant via cached conversation IDs — Q18)
...
### notifications (Pattern: private-own, system writes)
...
### device_tokens (Pattern: private-own)
...
### profile_embeddings, job_embeddings, ai_suggestions (Pattern: system-managed)
...
### saved_searches (Pattern: private-own)
...
### company_reviews (Pattern: public read, write-own)
...
| Total policies | ~65 |
| Access patterns | 6 (private-own, read-all-write-own, recruiter-chain, participant, lookup, system-managed) |
| Policy naming | `{table}.{role}.{operation}` dot-notation |
